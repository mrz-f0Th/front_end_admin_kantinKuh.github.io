...
export default {
  ...
  methods: {
    ...
    async tambahData() {
      ...
      // this.produk.gambar = this.image;
      try {
        await repository.putProduk(this.$route.params.id, formData);
  ...
  async mounted() {
    let { data } = await repository.getProdukId(this.$route.params.id);
    this.produk = data.data;
    this.preview = `http://localhost:8000/images/${data.data.gambar}`;
    this.image = `http://localhost:8000/images/${data.data.gambar}`;
